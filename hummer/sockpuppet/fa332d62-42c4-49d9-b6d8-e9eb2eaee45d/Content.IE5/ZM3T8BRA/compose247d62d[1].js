...
var _oTop=getTop();
_oTop.loadJsFileToTop(['$js_path$com/kits/qmeditor/qqmail/release/editor_toolbar_ext250c75.js']);
...
var _bIsNeedTransformNormalAttach2Big=(_oCompose.getInputObj("transattach").value==='true'||true),_oAttach2BigList=[];
...
if(_bIsNeedTransformNormalAttach2Big&&_aoFile.get('sSHA'))
{
_oAttach2BigList.push([encodeURIComponent(_aoFile.get('sName')),encodeURIComponent(_aoFile.get('nSize')),encodeURIComponent(_aoFile.get('sSHA')),encodeURIComponent(_aoFile.get('sMD5'))].join(','));
}
}
});
if(_bIsNeedTransformNormalAttach2Big&&_oAttach2BigList&&_oAttach2BigList.length>0)
{
var _oAttach2BigListInput=_oSelf.S("attach2BigList");
if(!_oAttach2BigListInput)
{
_oTop.insertHTML(_oCAInput,"afterEnd",_oTop.T('<input name="attach2BigList" type="hidden" id="attach2BigList" value="$value$" />').replace({value:_oAttach2BigList.join('|')}));
}
else{
_oAttach2BigListInput.value=_oAttach2BigList.join('|');
}
}
if(_oCAInput)
{
...
else if(/cgi,-8001,ftnCreatefile/.test(_sError))
{
_oEUploader.innerHTML="\u9644\u4EF6\u5185\u5BB9\u5F02\u5E38\uFF0C\u4E0A\u4F20\u5931\u8D25";
}
else if(/cgi,-8002,ftnCreatefile/.test(_sError))
{
_oEUploader.innerHTML="\u8BF7\u6C42\u7684\u9891\u7387\u592A\u5FEB\uFF0C\u8BF7\u7A0D\u540E\u518D\u8BD5";
}
...
try{
if(_asTemplate!="autosave"&&_asTemplate!="savedraft")
{
_fOnlineDocAddCollaborators();
}
}
catch(e)
{
_oTop.logKVOnlineDoc&&_oTop.logKVOnlineDoc("online_doc_add_collaborators_exception");
}
...
function _fOnlineDocAddCollaborators()
{
var _oAddrInput=_oCompose.getQMAddrInput();
var _sCollaborators=[];
var _oEditor=_oCompose.getPageEditor();
var _oOnlineDocKeys=_oEditor.getOnlineDocKeys&&_oEditor.getOnlineDocKeys();
if(!_oOnlineDocKeys||_oOnlineDocKeys.length<1)
{
return;
}
_oCompose.getInputObj("onlinedoc",null).value=1;
}
...
var _oTop=getTop();
_oTop.localReloadJsFile("$js_path$libcompose3e5857.js",function(){
...
_oProcess._nCurrentProcessId=90;
var _oTop=getTop();
if(_oTop.weworkSignature&&_oTop.goUserInfo)
{
var _oSignData=_oTop.goUserInfo.get&&_oTop.goUserInfo.get('RealAllSignature');
if(_oSignData)
{
_oTop.E(_oSignData,function(_aoItem,_sId){
if(_oTop.weworkSignature&&_oTop.weworkSignature.is(_aoItem[1],_aoItem[0]))
{
_oTop.weworkSignature.ossLog('qqmail_cardicon_send');
}
});
var _oEditor=_oCompose.getPageEditor();
var _oSignLinkEls=_oEditor&&_oEditor.getEditWin&&_oTop.finds('.weworkSignature_link',_oEditor.getEditWin());
if(_oSignLinkEls&&_oSignLinkEls.length)
{
_oTop.weworkSignature.ossLog('qqmail_card_send');
}
}
}
}};
...
{
_oTop.loadJsFileToTop(["$js_path$com/kits/qmeditor/qqmail/release/editor47d62d.js"]);
...
var _oSignData=getTop().goUserInfo.get('RealAllSignature');
if(false&&_oSignData)
{
var _sInitSignId=_oCompose._getEditorSignId("sign");
_oTop.E(_oSignData,function(_aoItem,_sId){
if(_sId!=_sInitSignId&&_oTop.weworkSignature&&_oTop.weworkSignature.is(_aoItem[1],_aoItem[0]))
{
_oTop.weworkSignature.refreshData(function(_aoErr,_aoData){
if(!_aoErr&&_aoData.mail_business_card_status==3)
{
if(!_oCompose._weworkSignatureTmpContent)
{
_oCompose._weworkSignatureTmpContent=_oTop.getSignatureHeader()+_oTop.weworkSignature.render(_aoData);
}
}
});
}
...
if(_oCompose.S('weworkSignatureTips'))
{
_oTop.QMAjax.send('/cgi-bin/readtemplate?t=get_buss_card&func=bitmap_wework_signature_compose&sid='+_oTop.getSid(),{onload:function(_abIsOk,_aoData){
if(!_abIsOk)
{
return;
}
var _oData=_oTop.evalValue(_aoData);
if(!_oData.value)
{
_oTop.show(_oCompose.S('weworkSignatureTips'),true);
_oTop.weworkSignature.ossLog('qqmail_card_bubble_show');
_oTop.QMAjax.send("/cgi-bin/do",{content:"action=setbitmap&key=wework_signature_compose&value=1",method:"POST",onload:function(_abIsOk){
}});
}
else{
_oTop.show(_oCompose.S('weworkSignatureTips'),false);
}
}});
}
}
...
_oCompose.autoSave(false);
_oCompose._setEditorPageEvent();
_oCompose.setMultiSignatureSelect();
...
_oTop.gbIsIE||this.loadFileToEditor({"$js_path$note_editor47d191.js":true});
var _sCSSCode=['ul.qmTaskList li{pointer-events:none;list-style-type:none;margin-left:25px;}ul.qmTaskList li:before{cursor:default;pointer-events:all;content:" ";margin-left:-25px;margin-right:10px;vertical-align:sub;display:inline-block;width:15px;height:15px;background:url($images_path$notepad/ico_tasklist475a0a.png) left top no-repeat;}ul.qmTaskList li.item-completed{pointer-events:none;color:#A2A2A7;text-align:left;opacity:0.6;filter:alpha(opacity=60);}br[type="_moz"]{margin-top:16px;margin-bottom:5px;}ul.qmTaskList li.item-completed:before{content:" ";background:url($images_path$notepad/ico_tasklist_completed475a0a.png) left top no-repeat;}@media only screen and (-webkit-min-device-pixel-ratio:2),only screen and (min-device-pixel-ratio:2){ul.qmTaskList li:before{background-image:url($images_path$notepad/ico_tasklist@2475a12.png);background-size:100%;}ul.qmTaskList li.item-completed:before{background-image:url($images_path$notepad/ico_tasklist_completed@2475a12.png);background-size:100%;}}'].join('\n');
_oTop.gbIsIE||_oTop.gbIsEdge||_oTop.evalCss(_sCSSCode,this.getEditDoc(),"notepad_full");
}
...
},setMultiSignatureSelect:function(){
var _oCompose=this;
var _oWin=_oCompose.getWin();
var _oTop=getTop();
try{
...
var _oWeworkSignatureTips=_oCompose.S('weworkSignatureTips');
var _bWeworkSignatureTips=!!_oWeworkSignatureTips;
...
{
var _oData=_oMenuDatas[_sId];
var _sContent=_oData[0];
if(typeof _sContent==='string')
{
if(_sContent.indexOf('class="xm_write_card"')>-1)
{
if(_sContent.indexOf('id="in_alias"')>-1)
{
var _sCurrentMail=_oTop.getUserInfo("addr")||_oTop.getCookie("qqmail_alias")||'';
if(_sContent.indexOf(_sCurrentMail)===-1)
{
continue;
}
else{
...
var _sTmpl=_oMenuItemTmpl.replace({sItemValue:_oData[1],bNewStyle:(_sId>199&&_sId<300)?true:false});
...
if(!_bWeworkSignatureTips&&!_buildMenuItems())
{
return;
}
_oTop.addEvent(_oContainer,"click",function(){
if(_oWeworkSignatureTips)
{
_oTop.removeSelf(_oWeworkSignatureTips);
_oWeworkSignatureTips=null;
}
...
{
var _sEmail=_oTop.getUin()+"@qq.com",_oLinkedIn=_oTop.bindLinkedInAccount;
...
else if(false&&_oTop.weworkSignature&&_oTop.weworkSignature.is(_oSign[1],_oSign[0]))
{
_sValue=_oCompose._weworkSignatureTmpContent||'';
_oTop.weworkSignature.refreshData(function(_aoErr,_aoData){
if(!_aoErr&&_aoData.mail_business_card_status==3)
{
var _bContentEdited=_oCompose._isContentEdited();
var _sNewValue=_oTop.getSignatureHeader()+_oTop.weworkSignature.render(_aoData);
if(_sNewValue!=_sValue||_oCompose._weworkSignatureTmpContent!=_sNewValue)
{
_oCompose._weworkSignatureTmpContent=_sNewValue;
_oCompose._setEditorSignValue(_asType,_sNewValue,_avParam,_abIsFocus);
if(!_bContentEdited&&_oCompose.getPageEditor())
{
_oCompose._oLoadValueCache[4]=_oCompose.getPageEditor().getContent();
}
