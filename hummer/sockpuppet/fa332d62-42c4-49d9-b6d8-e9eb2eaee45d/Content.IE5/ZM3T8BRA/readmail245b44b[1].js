...
},autoWaterMark:function(_aoContainer,_asMark){
var _oSelf=this;
var _oTop=window.getTop();
function toRadians(angle)
{
return angle*(Math.PI/180);
}
if(_oTop.gbIsIE||!_aoContainer)
{
return;
}
var _oReadmainContainer=_aoContainer;
var _oPosInfo=_oTop.calcPos(_oReadmainContainer,'json');
_aoContainer.style.minHeight=_oPosInfo.width/2*Math.sin(toRadians(20))+'px';
_oPosInfo=_oTop.calcPos(_oReadmainContainer,'json');
var _nNums=2*Math.ceil(_oPosInfo.height/300);
var _oLis=[];
for(var i=0,len=_nNums;i<len;i++)
{
_oLis.push(i+1);
}
var translateY=_oPosInfo.width/4*Math.sin(toRadians(15))-100;
var _sWaterMarkerHtml=_oTop.TE(['<div class="qm_watermark" style="width:$width$px;height:$height$px;">','$@$for($oLis$)$@$','<li class="qm_watermark_item" style="-webkit-transform:  translate(2%, ',translateY,'px) rotate(-15deg); -ms-transform: translate(2%, ',translateY,'px) rotate(-15deg);transform: translate(2%, ',translateY,'px) rotate(-15deg); ">$_root_.nickName$</li>','$@$endfor$@$','</div>']).replace({width:_oPosInfo.width,height:_oPosInfo.height,oLis:_oLis,nickName:_oTop.htmlEncode(_asMark||_oTop.getUserInfoText("alias")||_oTop.getUin())});
_oTop.insertHTML(_oReadmainContainer,"beforeEnd",_sWaterMarkerHtml);
var _fResize=function(){
var _oWaterMarkDoms=_oTop.finds('div.qm_watermark',_aoContainer);
if(_oWaterMarkDoms&&_oWaterMarkDoms.length>0)
{
_oWaterMarkDoms=_oWaterMarkDoms[0];
_oTop.removeSelf(_oWaterMarkDoms);
}
_oTop.removeEvent(_oTop.getMainWin(),'resize',_fResize);
_oTop.removeEvent(_oTop.getMainWin(),'load',_fResize);
_oSelf.autoWaterMark(_aoContainer,_asMark);
};
_oTop.addEvent(_oTop.getMainWin(),'resize',_fResize);
_oTop.addEvent(_oTop.getMainWin(),'load',_fResize);
},specialProcess:function(){
var _oSelf=this,_oTop=getTop(),containerDom,mailContentDom,contentRootDom;
var catemailtype='';
switch(_oSelf._msModuleName)
{case "qmConvMail":
catemailtype=_oSelf.getMailInfo().oSubMails[0][0].catemailtype;
...
var isBurnAfterReadEmail=(catemailtype==='kill'||_oSelf._moConfig.catemailtype==='kill');
var isBurnAfterTimerEmail=(catemailtype==='secret'||_oSelf._moConfig.catemailtype==='secret');
if(isBurnAfterReadEmail||isBurnAfterTimerEmail)
{
if(_oSelf._msModuleName!=='qmConvMail'&&_oSelf.S(_ID._sMailContainer))
{
containerDom=containerDom.parentNode;
}
_oSelf.autoWaterMark(containerDom);
if(isBurnAfterTimerEmail)
{
return;
}
_oTop.alertBox({confirmBtnTxt:'\u786E\u8BA4',msg:'\u8BE5\u90AE\u4EF6\u4E3A\u9605\u540E\u5373\u711A\u90AE\u4EF6\uFF0C\u79BB\u5F00\u540E\u5C06\u88AB\u711A\u6BC1',title:'\u9605\u540E\u5373\u711A\u90AE\u4EF6'});
_oTop.addEvent(_oSelf._moWin,'beforeunload',function(){
var _sMailId=_oSelf.getMailId();
if(_oSelf._msModuleName==="qmConvMail")
{
for(var _subMailId in _oSelf._moSubMails)
{
_sMailId=_subMailId;
break;
}
...
QMAjax.send('/cgi-bin/today?action=killmail',{content:_oTop.TE(["mailid=$mailid$"]).replace({mailid:_sMailId}),method:"POST",onload:function(_abIsOk,_asResult){
debug('killmail:',_sMailId,';status:',_abIsOk);
...
{
_oTop.E(_oADom,function(_asNode){
if(/\u4E0B\u8F7D/.test(_asNode.innerHTML))
{
_asNode.href+="&kvclick=attach|poison|download";
_asNode.onclick=function(_aoEvent){
...
_oTop.goUrlMainFrm(_asNode.href,false,true);
});
};
}
else if(/previewAttach/i.test(_oTop.attr(_asNode,'ck')))
{
_oTop.attr(_asNode,"ispoison","1");
}
});
}
}
if(_oImgPreview&&_oImgPreview.length>0)
{
_oTop.E(_oImgPreview,function(_asNode){
if(/previewAttach/i.test(_oTop.attr(_asNode,'ck')))
{
_oTop.attr(_asNode,"ispoison","1");
return false;
}
...
if(_oTop.weworkSignature&&_oTop.S('ww_signature_readmail',_oSelf._moWin))
{
_oTop.weworkSignature.ossLog('qqmail_cardicon_rece');
}
...
},getParse2OnlineDocUrl:function(_aoTarget){
var _oTop=getTop();
var sParse2OnlineDocUrl=_oTop.attr(_aoTarget,"sParse2OnlineDocUrl");
if(sParse2OnlineDocUrl&&!_oTop.gbIsIE)
{
var sDownLoadUrl=_oTop.attr(_aoTarget,"down");
return [sParse2OnlineDocUrl,"&preview=1","&downloadurl=",encodeURIComponent(sDownLoadUrl)].join("");
}
else{
return;
}
...
var sParse2OnlineDocUrl=_oSelf.getParse2OnlineDocUrl(_aoTarget);
sParse2OnlineDocUrl&&(_sUrl=sParse2OnlineDocUrl);
if(_oSelf.attr(_aoTarget,"ispoison")=="1")
{
...
_oTop.open('/cgi-bin/docedit_read?t=online_doc.html&key='+_sFileId+'&func=webread&sid='+_oTop.getSid(),"_blank");
...
{
_oLoadScriptFiles.push("$js_path$libcompose3e5857.js");
}
...
{
_afCallBack();
if(_sText.indexOf("<!--cgierrorcode:-304-->")>-1)
{
showError("\u90AE\u4EF6\u5DF2\u88AB\u711A\u6BC1");
}
else{
...
getTop().goExpers&&getTop().goExpers.isEncryptEmailAccount&&_oItems.push({sId:"forward",sItemValue:"\u8F6C\u53D1"});
_oItems.push(_oHr,{sId:"deleteMail",sItemValue:"\u5220\u9664"},{sId:"predeleteMail",sItemValue:"\u5F7B\u5E95\u5220\u9664"});
